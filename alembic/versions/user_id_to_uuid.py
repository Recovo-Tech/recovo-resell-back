...
def upgrade():
    ...
    op.drop_constraint(
        "second_hand_products_seller_id_fkey",
        "second_hand_products",
        type_="foreignkey",
    )
    # 2. Add new nullable UUID column to users
    op.add_column(
        "users", sa.Column("id_new", postgresql.UUID(as_uuid=True), nullable=True)
    )
    ...
    op.add_column(
        "carts", sa.Column("user_id_new", postgresql.UUID(as_uuid=True), nullable=True)
    )
    op.add_column(
        "second_hand_products",
        sa.Column("seller_id_new", postgresql.UUID(as_uuid=True), nullable=True),
    )
    ...
    for cart in carts:
        if cart[1] in id_map:
            conn.execute(
                sa.text("UPDATE carts SET user_id_new = :uuid WHERE id = :id"),
                {"uuid": id_map[cart[1]], "id": cart[0]},
            )
    shp = conn.execute(
        sa.text("SELECT id, seller_id FROM second_hand_products")
    ).fetchall()
    for product in shp:
        if product[1] in id_map:
            conn.execute(
                sa.text(
                    "UPDATE second_hand_products SET seller_id_new = :uuid WHERE id = :id"
                ),
                {"uuid": id_map[product[1]], "id": product[0]},
            )
    ...
    op.alter_column(
        "second_hand_products", "seller_id_new", new_column_name="seller_id"
    )
