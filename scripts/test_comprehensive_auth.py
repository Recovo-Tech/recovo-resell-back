...
def comprehensive_tenant_auth_test():
    ...
    try:
        ...
        if response.status_code in [200, 404]:  # 404 is OK if no products exist
            print(
                f"✅ Tenant-scoped endpoint accessible (status: {response.status_code})"
            )
        else:
    ...
    try:
        response = requests.post(
            f"{base_url}/auth/register", json=invalid_register_data
        )
        if response.status_code == 400:
            error_result = response.json()
            print(
                f"✅ Invalid tenant correctly rejected: {error_result.get('detail', 'No detail')}"
            )
        else:
