...
def test_shopify_products():
    ...
    try:
        response = requests.get(
            f"{base_url}/shopify/products/filters/available", headers=headers
        )
        if response.status_code == 200:
    ...
    try:
        response = requests.get(
            f"{base_url}/shopify/products/", headers=headers, params={"limit": 10}
        )
        if response.status_code == 200:
            ...
            print(
                f"   Pagination: Page {pagination.get('page', 1)}, Has next: {pagination.get('has_next_page', False)}"
            )
    ...
    if test_product_id:
        ...
        try:
            response = requests.get(
                f"{base_url}/shopify/products/{test_product_id}", headers=headers
            )
            if response.status_code == 200:
                ...
                if variants:
                    ...
                    for variant in variants[:3]:  # Show first 3 variants
                        print(
                            f"     - {variant.get('title', 'N/A')}: ${variant.get('price', 0)} (SKU: {variant.get('sku', 'N/A')})"
                        )
    ...
    print(f"\n🔎 Test 4: Search products")
    try:
        search_data = {"query": "shirt", "page": 1, "limit": 5}
        response = requests.post(
            f"{base_url}/shopify/products/search", headers=headers, json=search_data
        )
        if response.status_code == 200:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if collections:
                ...
                print(
                    f"   Testing with collection: {collections[0]['name']} (ID: {collection_id})"
                )
    ...
    try:
        advanced_search_data = {
            "query": "",
            "filters": {"status": "ACTIVE", "product_type": "T-Shirt"},
            "page": 1,
            "limit": 3,
        }
        response = requests.post(
            f"{base_url}/shopify/products/search",
            headers=headers,
        ...
        if response.status_code == 200:
            ...
            for i, product in enumerate(products, 1):
                print(
                    f"   {i}. {product['title']} (Type: {product.get('product_type', 'N/A')})"
                )
