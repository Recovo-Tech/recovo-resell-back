...
async def test_product_approval():
    ...
    try:
        ...
        if unapproved:
            ...
            print(
                f"Found unapproved product: ID={product_id}, Name='{name}', Shopify ID={shopify_id}"
            )

            # Try to approve it
            print("Attempting to approve product...")
            approval_result = await service.approve_product(product_id, tenant_id)
            ...
            if approval_result["success"]:
                ...
                if "shopify_product_id" in approval_result:
                    print(
                        f"🛍️ Shopify Product ID: {approval_result['shopify_product_id']}"
                    )

                if "message" in approval_result:
                    print(f"📋 Message: {approval_result['message']}")
            else:
                print(f"❌ Failed to approve product: {approval_result['error']}")
                print(f"  Error Code: {approval_result.get('error_code', 'N/A')}")
        else:
            print(
                "No unapproved products found. Let's check existing approved products."
            )
            ...
            for product in approved_products:
                print(
                    f"  - ID: {product[0]}, Name: '{product[2]}', Shopify ID: {product[4]}"
                )
