...
def test_shopify_categories():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, category in enumerate(categories[:3]):
                print(
                    f"   {i+1}. {category['name']} (ID: {category['id']}) - {category['products_count']} products"
                )
    ...
    if first_category_id:
        ...
        try:
            response = requests.get(
                f"{base_url}/shopify/categories/{first_category_id}", headers=headers
            )
            if response.status_code == 200:
                result = response.json()
                ...
                print(
                    f"   Description: {category['description'][:100]}..."
    ...
    try:
        response = requests.get(
            f"{base_url}/shopify/categories/search/{search_term}", headers=headers
        )
        if response.status_code == 200:
            result = response.json()
            categories = result.get("categories", [])
