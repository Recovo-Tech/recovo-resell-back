...
async def test_improvements():
    ...
    try:
        ...
        if products_with_data:
            print(
                f"Found {len(products_with_data)} products with weight and original info:"
            )
            ...
            print(
                "❌ No products found with weight and original info. Creating test data..."
            )
        ...
        if not test_product:
            ...
            print(
                "ℹ️ No unapproved products, testing with approved product (should show re-approval handling)"
            )
        ...
        print(
            f"Result: Success={fake_result['success']}, Error='{fake_result.get('error', 'N/A')}', Code={fake_result.get('error_code', 'N/A')}"
        )
        ...
        print(
            f"Result: Success={invalid_result['success']}, Error='{invalid_result.get('error', 'N/A')}', Code={invalid_result.get('error_code', 'N/A')}"
        )
...
async def verify_shopify_product(
    tenant_id,
    shopify_product_id,
    expected_weight,
        ...
        if result and "data" in result and result["data"]["product"]:
            ...
            if product.get("variants", {}).get("edges"):
                ...
                if expected_weight_unit and actual_weight_unit:
                    if actual_weight_unit.upper() == expected_weight_unit.upper():
                        ...
                        print(
                            f"     ❌ Weight unit mismatch: expected {expected_weight_unit}, got {actual_weight_unit}"
                        )
            ...
            if expected_type and actual_type:
                if actual_type == expected_type:
                    ...
                    print(
                        f"     ❌ Product type mismatch: expected '{expected_type}', got '{actual_type}'"
                    )
            ...
            if expected_vendor and actual_vendor:
                if actual_vendor == expected_vendor:
                    ...
                    print(
                        f"     ❌ Vendor mismatch: expected '{expected_vendor}', got '{actual_vendor}'"
                    )
