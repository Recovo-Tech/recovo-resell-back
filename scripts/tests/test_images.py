...
async def test_images_upload():
    ...
    try:
        ...
        product = db.query(SecondHandProduct).filter(
            SecondHandProduct.id >= 3  # Products 3+ are our test products without images
        ).first()
        ...
        current_images = db.query(SecondHandProductImage).filter(
            SecondHandProductImage.product_id == product.id
        ).all()
        ...
        updated_images = db.query(SecondHandProductImage).filter(
            SecondHandProductImage.product_id == product.id
        ).all()
