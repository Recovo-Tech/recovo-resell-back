...
async def test_image_upload_approaches():
    ...
    try:
        ...
        print(f"🧪 Testing image upload approaches for tenant: {tenant.shopify_app_url}")
        ...
        # Summary
        print("\n📊 SUMMARY:")
        print(f"   Method 1 (productSet with files): {'✅ Success' if success1 else '❌ Failed'}")
        print(f"   Method 2 (productCreate + media): {'✅ Success' if success2 else '❌ Failed'}")
        
        print("\n💡 RECOMMENDATIONS:")
        if success1 and success2:
            print("   Both methods work. productSet is more modern and should be preferred.")
        elif success1:
            print("   Use productSet with files - it's the modern approach.")
        elif success2:
            print("   Use productCreate + productCreateMedia as fallback.")
        else:
            print("   Both methods failed - investigate API credentials and image URLs.")
